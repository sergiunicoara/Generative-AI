    ...
    try:
        with sync_playwright() as p:
            ...
            for label, slug in TABS:
                ...
                # Click the tab unless it's already the default view.
                try:
                    page.get_by_text(label, exact=True).first.click(timeout=4000)
                except Exception:
                    pass
                page.wait_for_timeout(1500)  # let the tab content swap in
                # Plotly charts only exist on some tabs; wait if present.
                try:
                    page.wait_for_selector(".js-plotly-plot", timeout=4000)
                except Exception:
                    pass
                page.wait_for_timeout(2000)  # let charts/tables finish sizing
