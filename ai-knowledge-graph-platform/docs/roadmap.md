...
| PROV-O RDF alignment (`graphrag/provenance/prov_o.py`, `scripts/export_rdf.py`) | **Implemented and SHACL-validated** | Tenant-scoped documents, chunks, assertions, ingestion activities, retrieval activities, software agents, evidence inputs, and answer digests are exported with `prov:used`, `prov:wasGeneratedBy`, `prov:wasDerivedFrom`, and `prov:wasAssociatedWith`; source text is not copied into the PROV projection. |
