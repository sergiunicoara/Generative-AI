...
class IngestionAgent(BaseGraphRAGAgent):
    ...
    async def write(self, extracted: dict) -> dict:
        ...
        # CorpusMutation guarantees complete_corpus_update runs on the exception
        # path too, so active_updates never leaks past a failed write (see
        # docs/archive/audits/audit-2026-09-23.md, "Not fixed" #1).
        async with CorpusMutation(self._writer.neo4j_client, doc.tenant, reason="ingestion") as mutation:
            ...
            # evidence existing, not on whether write_document happened to remap
            # the id this run (see audit-2026-09-23.md, "Not fixed" #3).
            has_prior_evidence = await self._writer.document_has_evidence(
